...
const ReportePagos = ({ reportesRESIDENCIA }) => {

    const bgChart = useColorModeValue('white', '#1e1e1e');

    var pagosRESIDENCIA = [];

    if (reportesRESIDENCIA?.pagos) {
        pagosRESIDENCIA = reportesRESIDENCIA?.pagos[0];
    }
    ...
    const BarOptions = {
        ...
        series: [
            {
                name: 'AÑO ACTUAL',
                data: [pagosRESIDENCIA?.cantidad_pagos_por_anio],
                color: '#3498db'
            },
            {
                name: 'ESTE MES',
                data: [pagosRESIDENCIA?.cantidad_pagos_por_mes],
                color: '#2ecc71'
            },
            {
                name: 'TOTAL DE REGISTROS',
                data: [pagosRESIDENCIA?.total_registro_pagos],
                color: '#4a5568'
            },
        ],
        exporting: {
        ...
        series: [
            {
                ...
                data: [pagosRESIDENCIA?.monto_total_pagos_por_anio],
                color: '#154360'
            },
            {
                name: 'MONTO TOTAL GENERAL',
                data: [pagosRESIDENCIA?.monto_total_pagos],
                color: '#5dade2'
            }
        ],
        exporting: {
            ...
            data: [
                {
                    ...
                    y: pagosRESIDENCIA?.pagos_cancelados,
                    color: '#38a169'
                },
                {
                    name: 'PAGOS PENDIENTES',
                    y: pagosRESIDENCIA?.pagos_pendientes,
                    color: '#e53e3e'
                },
                {
                    name: 'PAGOS INCOMPLETOS',
                    y: pagosRESIDENCIA?.pagos_incompletos,
                    color: '#f1c40f'
                }
            ]
        }],
    ...
    const Barras = {
        ...
        series: [
            {
                name: 'EFECTIVO',
                data: [pagosRESIDENCIA?.pagos_efectivo],
                color: '#e53e3e'
            },
            {
                name: 'TARJETA DE CRÉDITO',
                data: [pagosRESIDENCIA?.pagos_tarjeta],
                color: '#3498db'
            },
            {
                name: 'TRANSFERENCIA BANCARIA',
                data: [pagosRESIDENCIA?.pagos_transferencia],
                color: '#34495e'
            },
            {
                name: 'YAPE',
                data: [pagosRESIDENCIA?.pagos_yape],
                color: '#7d3c98'
            },
            {
                name: 'DEPOSITO BANCARIO',
                data: [pagosRESIDENCIA?.pagos_deposito],
                color: '#d4ac0d'
            },
            {
                name: 'OTROS',
                data: [pagosRESIDENCIA?.pagos_otros],
                color: '#b03a2e'
            },
        ]
    }
